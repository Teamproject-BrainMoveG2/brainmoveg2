...
@router.post("/hit")
async def record_cone_hit(cone: ConeDTO, game_service: GameService = Depends(get_game_service), sio=Depends(get_sio), cone_service: ConeService = Depends(get_cone_service), score_service: ScoreService = Depends(get_score_service)):
    try:
        await game_service.record_round(cone.cone_id, sio, cone_service, score_service)
