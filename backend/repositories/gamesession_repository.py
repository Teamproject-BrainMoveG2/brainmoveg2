...
class GameSessionRepository:
    ...
    def get_sessions_in_date_range(settings, start_date, end_date):
        sql = "SELECT COUNT(spelronde.spelronde_id) as aantal_rondes, spelsessie.spelsessie_id, username, spelmodus.naam as spelmodus, moeilijkheid.naam as moeilijkheid, score, AVG(spelronde.reactietijd_ms) AS avg_reactietijd_ms, SUM(CASE WHEN spelronde.uitkomst = 'goed' THEN 1 ELSE 0 END) * 100.0 / COUNT(*) AS accuracy_percent, gestart_op as datum FROM spelsessie LEFT JOIN moeilijkheid ON spelsessie.moeilijkheid_id = moeilijkheid.moeilijkheid_id INNER JOIN spelronde ON spelsessie.spelsessie_id = spelronde.spelsessie_id INNER JOIN spelmodus on spelsessie.spelmodus_id = spelmodus.spelmodus_id WHERE DATE(spelsessie.gestart_op) BETWEEN %s AND %s GROUP BY spelsessie.spelsessie_id, username, spelsessie.spelmodus_id, moeilijkheid.naam, score"
